...
// Latency summary (use timing values even if fetch is opaque/fails)
...
// Transport looks OK if ANY probe completed
const transportOk = [g204, cfTrace, cfHome].some((p) => p?.ok);

// If online + transport works, but captive probe errors/is very slow → suspect captive portal
const captiveSuspected =
  online &&
  transportOk &&
  (captiveProbe.ok === false || captiveProbe.ms >= 1800);

// DNS heuristic: if transport works but domain probes fail, suspect DNS/APN/VPN/Private DNS
...
return {
  ...
  latency: {
    google204: g204,
    cloudflare: cfTrace, // keep this if your UI still reads latency.cloudflare
    cfHome,
    bestMs,
    worstMs,
    note:
